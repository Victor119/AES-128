...
void generate_random_iv(unsigned char* iv, int length) {
...
char* encrypt(const char* message, const char* key, unsigned char* iv) {
    int i=0, j=0, k=0;
    ...
    char** iv_state = (char**)malloc(rows * sizeof(char*));
    for (i = 0; i < rows; i++) {
        iv_state[i] = (char*)malloc(cols * sizeof(char));
    }
    ...
    for(j = 0; j < cols; j++) {
        for(i = 0; i < rows; i++) {
            if (counter2 < size_iv_message){
                iv_state[i][j] = iv[counter2];
                counter2++;
            }
            else {
                iv_state[i][j] = 0; //padding
            }
        }
    }
    ...
    //binary state
    char*** iv_binary_state = (char***)malloc(rows * sizeof(char**));
    for (i = 0; i < rows; i++) {
        iv_binary_state[i] = (char**)malloc(cols * sizeof(char*));
        for (j = 0; j < cols; j++) {
            iv_binary_state[i][j] = NULL;
        }
    }

    printf("iv_binary_state: ");
    printf("\n");
    for(i = 0; i < rows; i++) {
        for(j = 0; j < cols; j++){
            int dec_value = (unsigned char)iv_state[i][j];
            iv_binary_state[i][j] = decToBinary(dec_value);
            printf("%s ", iv_binary_state[i][j]);
        }
        printf("\n");
    }
    ...
    for(i = 0; i < rows; i++) {
        for(j = 0; j < cols; j++){
            int val1 = BinaryStringToInt(binary_state[i][j]);
            int val2 = BinaryStringToInt(iv_binary_state[i][j]);
            int xor_result = val1 ^ val2;

            if (binary_state[i][j] != NULL) {
                free(binary_state[i][j]);
            }

            binary_state[i][j] = decToBinary(xor_result);
            printf("%s ", binary_state[i][j]);
        }
        printf("\n");
    }
    ...
    for(i = 0; i < rows; i++) {
        free(iv_state[i]);
        for(j = 0; j < cols; j++) {
            if (iv_binary_state[i][j] != NULL) {
                free(iv_binary_state[i][j]);
            }
        }
        free(iv_binary_state[i]);
    }
    free(iv_state);
    free(iv_binary_state);
    ...
    // Convertim binary_state intr-un sir hexazecimal
    char* result = (char*)malloc(33); // 16 bytes * 2 chars + null terminator
    result[0] = '\0';
    
    for(j = 0; j < cols; j++) {
        for(i = 0; i < rows; i++) {
            int val = BinaryStringToInt(binary_state[i][j]);
            char hex[3];
            sprintf(hex, "%02x", val);
            strcat(result, hex);
        }
    ...
    for (i = 0; i < rows; i++) {
        for (j = 0; j < cols; j++) {
            if (binary_state_key[i][j] != NULL) {
                free(binary_state_key[i][j]);
            }
        }
        free(binary_state_key[i]);
    }
    free(binary_state_key);
...
int main() {
    ...
    char *encrypted = encrypt(message, key, iv);
    printf("\nEncrypted message: %s\n", encrypted);
